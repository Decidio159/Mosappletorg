...
(function () {
  ...
  function завести(обёртка) {
    ...
    function листнуть(куда) {
      ...
      var п = половина(), ш = шаг();
      едем = Date.now() + 700;

      /* Перескок делаем ДО шага, а не после. Если ждать, пока прокрутка сама
         доедет до конца копии, человек успевает увидеть край ленты и пустое
         поле за последней карточкой. Так он всегда листает «в середине». */
      if (куда > 0 && п && трек.scrollLeft + ш >= п) {
        трек.scrollLeft = Math.max(0, трек.scrollLeft - п);
      }
      /* Назад с первой карточки: сначала мгновенно уходим в конец копии,
         и уже оттуда едем влево — движение честное, без перемотки. */
      if (куда < 0 && трек.scrollLeft <= 2) {
        трек.scrollLeft += п || (трек.scrollWidth - трек.clientWidth);
      }
      трек.scrollBy({ left: куда * ш, behavior: плавно });
    }
